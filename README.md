...
`config.yml` currently selects **Random Forest**, so that is the model saved in
`models/model.pkl` and running in the deployed API. Bold marks the best value in
each column: Random Forest wins RMSE, MAE and R2, LightGBM wins MAPE.
